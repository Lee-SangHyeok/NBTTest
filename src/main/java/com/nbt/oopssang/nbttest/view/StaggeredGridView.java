...
public class StaggeredGridView extends ViewGroup {
    ...
    private STGVAdapter.HeaderFooterListAdapter mAdapter;
...
//                final int colEnd = Math.min(mColCount, nextCol + lp.span);
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        clearAllState();
        mAdapter = new STGVAdapter.HeaderFooterListAdapter(mHeaderView, mFooterView, adapter);
        mDataChanged = true;
            ...
            if (this.mColWidth > 0)
                for (int i = 0; i < mColCount; i++) {
                    if (getChildAt(i) != null) {
                        ...
                        final int left = child.getLeft();
                        int col = 0;
                        Log.d("mColWidth", mColWidth + " " + left);
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        public LayoutParams(int height) {
            ...
            if (this.height == MATCH_PARENT) {
                Log.d(TAG, "Constructing LayoutParams with height FILL_PARENT - " +
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            if (this.width != MATCH_PARENT) {
                Log.d(TAG, "Inflation setting LayoutParams width to " + this.width +
                        " - must be MATCH_PARENT");
                this.width = MATCH_PARENT;
            }
            if (this.height == MATCH_PARENT) {
                Log.d(TAG, "Inflation setting LayoutParams height to MATCH_PARENT - " +
                        "impossible! Falling back to WRAP_CONTENT");
                this.height = WRAP_CONTENT;
            }
        ...
        public LayoutParams(ViewGroup.LayoutParams other) {
            ...
            if (this.width != MATCH_PARENT) {
                Log.d(TAG, "Constructing LayoutParams with width " + this.width +
                        " - must be MATCH_PARENT");
                this.width = MATCH_PARENT;
            }
            if (this.height == MATCH_PARENT) {
                Log.d(TAG, "Constructing LayoutParams with height MATCH_PARENT - " +
                        "impossible! Falling back to WRAP_CONTENT");
                this.height = WRAP_CONTENT;
            }
        }
